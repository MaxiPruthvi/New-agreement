...
def get_db_connection():
    return mysql.connector.connect(
        ...
        user=os.environ.get('DB_USER', 'sql12742512'),
        password=os.environ.get('DB_PASSWORD', 'yq66lhrjBv'),
        database=os.environ.get('DB_NAME', 'sql12742512'),
...
def agreements():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT Agreement_No, Customer_Name, Agreement_Value FROM PA_Header")  # Ensure to select Customer_Name
